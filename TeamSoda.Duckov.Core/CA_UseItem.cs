...
public class CA_UseItem : CharacterActionBase, IProgress
{
	...
	protected override void OnStop()
	{
		...
		if (this.item != null && !this.item.IsBeingDestroyed && ItemTreeExtensions.GetRoot(this.item) != this.characterController.CharacterItem && !this.characterController.PickupItem(this.item))
		{
	...
	private void OnFinish()
	{
		...
		else if (this.item.UseDurability)
		{
			...
			{
				ItemTreeExtensions.DestroyTree(this.item);
			}
		}
		else
		{
	...
	private void StopSound()
	{
		...
		{
			this.soundInstance.Value.stop(0);
		}
	}
