using System;
using Unity.Collections;
using Unity.Mathematics;
using UnityEngine;

namespace Pathfinding
{
	// Token: 0x02000063 RID: 99
	public class GraphUpdateShape
	{
		// Token: 0x170000A7 RID: 167
		// (get) Token: 0x0600037C RID: 892 RVA: 0x00010EF2 File Offset: 0x0000F0F2
		// (set) Token: 0x0600037D RID: 893 RVA: 0x00010EFA File Offset: 0x0000F0FA
		public Vector3[] points
		{
			get
			{
				return this._points;
			}
			set
			{
				this._points = value;
				if (this.convex)
				{
					this.CalculateConvexHull();
				}
			}
		}

		// Token: 0x170000A8 RID: 168
		// (get) Token: 0x0600037E RID: 894 RVA: 0x00010F11 File Offset: 0x0000F111
		// (set) Token: 0x0600037F RID: 895 RVA: 0x00010F19 File Offset: 0x0000F119
		public bool convex
		{
			get
			{
				return this._convex;
			}
			set
			{
				if (this._convex != value && value)
				{
					this.CalculateConvexHull();
				}
				this._convex = value;
			}
		}

		// Token: 0x06000380 RID: 896 RVA: 0x00010F38 File Offset: 0x0000F138
		public GraphUpdateShape()
		{
		}

		// Token: 0x06000381 RID: 897 RVA: 0x00010F64 File Offset: 0x0000F164
		public GraphUpdateShape(Vector3[] points, bool convex, Matrix4x4 matrix, float minimumHeight)
		{
			this.convex = convex;
			this.points = points;
			this.origin = matrix.MultiplyPoint3x4(Vector3.zero);
			this.right = matrix.MultiplyPoint3x4(Vector3.right) - this.origin;
			this.up = matrix.MultiplyPoint3x4(Vector3.up) - this.origin;
			this.forward = matrix.MultiplyPoint3x4(Vector3.forward) - this.origin;
			this.minimumHeight = minimumHeight;
		}

		// Token: 0x06000382 RID: 898 RVA: 0x00011017 File Offset: 0x0000F217
		private void CalculateConvexHull()
		{
			this._convexPoints = ((this.points != null) ? Polygon.ConvexHullXZ(this.points) : null);
		}

		// Token: 0x06000383 RID: 899 RVA: 0x00011035 File Offset: 0x0000F235
		public Bounds GetBounds()
		{
			return GraphUpdateShape.GetBounds(this.convex ? this._convexPoints : this.points, this.right, this.up, this.forward, this.origin, this.minimumHeight);
		}

		// Token: 0x06000384 RID: 900 RVA: 0x00011070 File Offset: 0x0000F270
		public static Bounds GetBounds(Vector3[] points, Matrix4x4 matrix, float minimumHeight)
		{
			Vector3 b = matrix.MultiplyPoint3x4(Vector3.zero);
			Vector3 vector = matrix.MultiplyPoint3x4(Vector3.right) - b;
			Vector3 vector2 = matrix.MultiplyPoint3x4(Vector3.up) - b;
			Vector3 vector3 = matrix.MultiplyPoint3x4(Vector3.forward) - b;
			return GraphUpdateShape.GetBounds(points, vector, vector2, vector3, b, minimumHeight);
		}

		// Token: 0x06000385 RID: 901 RVA: 0x000110D0 File Offset: 0x0000F2D0
		private static Bounds GetBounds(Vector3[] points, Vector3 right, Vector3 up, Vector3 forward, Vector3 origin, float minimumHeight)
		{
			if (points == null || points.Length == 0)
			{
				return default(Bounds);
			}
			float num = points[0].y;
			float num2 = points[0].y;
			for (int i = 0; i < points.Length; i++)
			{
				num = Mathf.Min(num, points[i].y);
				num2 = Mathf.Max(num2, points[i].y);
			}
			float num3 = Mathf.Max(minimumHeight - (num2 - num), 0f) * 0.5f;
			num -= num3;
			num2 += num3;
			Vector3 vector = right * points[0].x + up * points[0].y + forward * points[0].z;
			Vector3 vector2 = vector;
			for (int j = 0; j < points.Length; j++)
			{
				Vector3 a = right * points[j].x + forward * points[j].z;
				Vector3 rhs = a + up * num;
				Vector3 rhs2 = a + up * num2;
				vector = Vector3.Min(vector, rhs);
				vector = Vector3.Min(vector, rhs2);
				vector2 = Vector3.Max(vector2, rhs);
				vector2 = Vector3.Max(vector2, rhs2);
			}
			return new Bounds((vector + vector2) * 0.5f + origin, vector2 - vector);
		}

		// Token: 0x06000386 RID: 902 RVA: 0x0001124E File Offset: 0x0000F44E
		public bool Contains(GraphNode node)
		{
			return this.Contains((Vector3)node.position);
		}

		// Token: 0x06000387 RID: 903 RVA: 0x00011264 File Offset: 0x0000F464
		public bool Contains(Vector3 point)
		{
			point -= this.origin;
			Vector3 p = new Vector3(Vector3.Dot(point, this.right) / this.right.sqrMagnitude, 0f, Vector3.Dot(point, this.forward) / this.forward.sqrMagnitude);
			if (!this.convex)
			{
				return this._points != null && Polygon.ContainsPointXZ(this._points, p);
			}
			if (this._convexPoints == null)
			{
				return false;
			}
			int i = 0;
			int num = this._convexPoints.Length - 1;
			while (i < this._convexPoints.Length)
			{
				if (VectorMath.RightOrColinearXZ(this._convexPoints[i], this._convexPoints[num], p))
				{
					return false;
				}
				num = i;
				i++;
			}
			return true;
		}

		// Token: 0x0400021C RID: 540
		private Vector3[] _points;

		// Token: 0x0400021D RID: 541
		private Vector3[] _convexPoints;

		// Token: 0x0400021E RID: 542
		private bool _convex;

		// Token: 0x0400021F RID: 543
		private Vector3 right = Vector3.right;

		// Token: 0x04000220 RID: 544
		private Vector3 forward = Vector3.forward;

		// Token: 0x04000221 RID: 545
		private Vector3 up = Vector3.up;

		// Token: 0x04000222 RID: 546
		private Vector3 origin;

		// Token: 0x04000223 RID: 547
		public float minimumHeight;

		// Token: 0x02000064 RID: 100
		public struct BurstShape
		{
			// Token: 0x06000388 RID: 904 RVA: 0x00011328 File Offset: 0x0000F528
			public BurstShape(GraphUpdateShape scene, Allocator allocator)
			{
				Vector3[] array = scene.convex ? scene._convexPoints : scene._points;
				if (array == null)
				{
					this.points = new NativeArray<Vector3>(0, allocator, NativeArrayOptions.ClearMemory);
				}
				else
				{
					this.points = new NativeArray<Vector3>(array, allocator);
				}
				this.origin = scene.origin;
				this.right = scene.right;
				this.forward = scene.forward;
				float sqrMagnitude = scene.right.sqrMagnitude;
				if (sqrMagnitude > 0f)
				{
					this.right /= sqrMagnitude;
				}
				sqrMagnitude = scene.forward.sqrMagnitude;
				if (sqrMagnitude > 0f)
				{
					this.forward /= sqrMagnitude;
				}
				this.containsEverything = false;
			}

			// Token: 0x170000A9 RID: 169
			// (get) Token: 0x06000389 RID: 905 RVA: 0x000113F4 File Offset: 0x0000F5F4
			public static GraphUpdateShape.BurstShape Everything
			{
				get
				{
					return new GraphUpdateShape.BurstShape
					{
						points = new NativeArray<Vector3>(0, Allocator.Persistent, NativeArrayOptions.ClearMemory),
						origin = float3.zero,
						right = float3.zero,
						forward = float3.zero,
						containsEverything = true
					};
				}
			}

			// Token: 0x0600038A RID: 906 RVA: 0x00011448 File Offset: 0x0000F648
			public bool Contains(float3 point)
			{
				if (this.containsEverything)
				{
					return true;
				}
				point -= this.origin;
				float3 @float = new float3(math.dot(point, this.right), 0f, math.dot(point, this.forward));
				int index = this.points.Length - 1;
				bool flag = false;
				int i = 0;
				while (i < this.points.Length)
				{
					if (((this.points[i].z <= @float.z && @float.z < this.points[index].z) || (this.points[index].z <= @float.z && @float.z < this.points[i].z)) && @float.x < (this.points[index].x - this.points[i].x) * (@float.z - this.points[i].z) / (this.points[index].z - this.points[i].z) + this.points[i].x)
					{
						flag = !flag;
					}
					index = i++;
				}
				return flag;
			}

			// Token: 0x04000224 RID: 548
			[DeallocateOnJobCompletion]
			private NativeArray<Vector3> points;

			// Token: 0x04000225 RID: 549
			private float3 origin;

			// Token: 0x04000226 RID: 550
			private float3 right;

			// Token: 0x04000227 RID: 551
			private float3 forward;

			// Token: 0x04000228 RID: 552
			private bool containsEverything;
		}
	}
}
